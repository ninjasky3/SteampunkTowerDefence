...
public class BuildingTurretRight : MonoBehaviour {
	...
	void Start () {
		...
		if(createOnLoad){
			GameObject thisturret =	Instantiate(Resources.Load("Turret 1"), transform.position, 
	...
	void OnGUI() {
		...
		if (window2){
			...
			if (turretbought && buildingbought && money > 149){
				turret.GetComponent<Turretshoot>().upgrading();
					...
					turret.GetComponent<AnimatioMovement>().cols = 6;
					turret.GetComponent<AnimatioMovement>().rows = 15;
					turret.GetComponent<AnimatioMovement>().cells = 60;
				window2 = false;
		...
		if (window3){
			...
			if (GUI.Button(new Rect(10, 10, 150, 100), "",upgradegui)){
				if (turretbought && buildingbought && money > 249){
					...
					turret.GetComponent<AnimatioMovement>().cols = 8;
					turret.GetComponent<AnimatioMovement>().rows = 12;
					turret.GetComponent<AnimatioMovement>().cells = 96;
					window2 = false;
					buildingslot.GetComponent<MoneyScript>().money -= 249;
					canbuy2 = true;
		...
		if(window4)
		{
			...
			{
				Instantiate(Resources.Load("RocketTurret 1"), transform.position, transform.rotation);
				rocketbought = true;
				turretbought = true;
				window4 	 = false;
				window1 = false;
